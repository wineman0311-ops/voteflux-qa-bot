...
def run_analysis_sync(version: str = "") -> tuple:
    ...
    try:
        ...
        if platforms_data is not None:
            logger.info(
                f"Using today's cached scrape data "
                f"({len(platforms_data)} platforms, {len(countries_data)} countries)"
            )
            used_cache = True
        else:
            logger.info("No today's cache found — running fresh scrape")
            used_cache = False
            ...
            if platforms_data or countries_data:
                if cache.save_today_cache(platforms_data, countries_data):
                    logger.info("Fresh scrape data saved to cache")
                else:
                    logger.warning("Failed to save scrape data to cache")
